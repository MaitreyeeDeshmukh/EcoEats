...
import {
  SignOut, PencilSimple, Check, X, Star, Plus, HandHeart, Student, ArrowsLeftRight,
...
function HistoryItem({ item, isOrganizer }) {
  if (isOrganizer) {
    const statusColors = {
      ...
      {isOrganizer ? null : (
        <div className="mx-4 bg-white rounded-card shadow-card p-4 mb-4">
          ...
          {claims.length === 0 ? (
            ...
            claims.slice(0, 10).map((c) => <HistoryItem key={c.id} item={c} isOrganizer={false} />)
          )}
        </div>
      )}
    </div>
